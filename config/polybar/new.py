...
if __name__ == '__main__':
    ...
    # process checks
    parser.add_argument("--parent", choices=list(PROCESS_TYPE_CHECKERS.keys()), nargs='?', help="check for a specific parent process")
    parser.add_argument("--client", choices=list(PROCESS_TYPE_CHECKERS.keys()), nargs='?', help="check for a specific (network) client process")
    parser.add_argument("--stdin_pipe", choices=list(PROCESS_TYPE_CHECKERS.keys()), nargs='?', help="check for a specific process at the other end of stdin")
    parser.add_argument("--stdout_pipe", choices=list(PROCESS_TYPE_CHECKERS.keys()), nargs='?', help="check for a specific process at the other end of stdout")
    parser.add_argument("--stdin_parent", action='store_true', help="make sure the parent is communicating with us over stdin")
    parser.add_argument("--stdout_parent", action='store_true', help="make sure the parent is communicating with us over stdout")

    # i/o
    parser.add_argument("--listen_dup", type=int, nargs='?', help="port to listen for input on")
    parser.add_argument("--input_dup", type=int, nargs='?', help="some crazy FD to take input on")
    parser.add_argument("--stdin_path", type=str, nargs='?', help="check for a specific input path")
    parser.add_argument("--stdout_path", type=str, nargs='?', help="check for a specific output path")
    parser.add_argument("--stderr_path", type=str, nargs='?', help="check for a specific stderr path")
    parser.add_argument("--stdin_fifo", action='store_true', help="make sure stdin is a fifo")
    parser.add_argument("--stdout_fifo", action='store_true', help="make sure stdout is a fifo")

    # other process stuff
    parser.add_argument("--cwd", type=str, nargs='?', help="check for a specific working directory")
    parser.add_argument("--parent_different_cwd", action='store_true', help="force parent CWD to be different than our CWD")
    parser.add_argument("--empty_env", action='store_true', help="check that the environment is empty (except LC_CTYPE)")
    parser.add_argument("--empty_argv", action='store_true', help="check that the argv is empty")

    # arg stuff
    parser.add_argument("--check_arg", type=str, nargs='?', help="NUM:VALUE, check argv[NUM] for value VALUE")
    parser.add_argument("--check_env", type=str, nargs='?', help="KEY:VALUE, check env[KEY] for value VALUE")

    # challenges
    parser.add_argument("--num_challenges", type=int, nargs='?', help="number of challenges")
    parser.add_argument("--challenge_ops", type=str, default="+", nargs='?', help="types of challenges")
    parser.add_argument("--challenge_depth", type=int, default=1, nargs='?', help="depth of each challenge AST")
    parser.add_argument("--password", type=str, nargs='?', help="check for a simple password")
    parser.add_argument("--num_signals", type=int, nargs='?', help="number of signals to wait for")
